...
def difference_luminance_images(lum1_path_and_name, lum2_path_and_name, in1_fname, in2_fname, output_directory, indates_lumin, i_lumin1, i_lumin2):
    ...
    deltaLum = np.subtract(sec_image, ref_image)  ## secondary - reference; the greater reduction, the greater the difference; more negative deltaLum --> greater change due to fire. 
    ...
    deltaLum_outfilename = tmpName.replace("RGBLumin","DeltaLumin")
    ...
    write_DeltaLum_metafile(deltaLum_logfile_and_path, in1_path, in1_fname, in2_path, in2_fname, diff_eq)
     
    return
    # return deltaLum, image_profile
...
def write_DeltaLum_metafile(deltaLum_logfile_and_path, in1_path, in1_fname, in2_path, in2_fname, diff_eq):
    Line1 = ( "Delta Luminance log file and path: " + deltaLum_logfile_and_path + "\n")
    Line2 = ( "Lum Image 1 Input filename: " + in1_fname + "\n")
    Line3 = ( "Lum Image 1 Input Path: " + in1_path + "\n")
    Line4 = ( "Lum Image 2 Input filename: " + in2_fname + "\n")
...
def parse_input_data(p):
    ...
    lum_dir = args['lum_file_loc']  
    output_directory = args['out_dir']
    lum_suffix = args['lumin_suffix']
    coh_change_dir = args['coh_change_file_loc']
    coh_change_suffix = args['coh_change_suffix']
    lum_ref_img_fname = args['lum_ref_image']
...
if __name__ == "__main__":
    ...
    for n in range(0, len(coh_images)):
        ...
        if len(i_lumin1) < 1 or len(i_lumin2) < 1:
            ...
            # deltaLum, image_profile = difference_luminance_images(luminance_images_norm[i_lumin1], luminance_images_norm[i_lumin2], in1_fname, in2_fname, output_directory)            
