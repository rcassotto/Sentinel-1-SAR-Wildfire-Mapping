...
def difference_luminance_images(lum1_path_and_name, lum2_path_and_name, in1_fname, in2_fname, output_directory, indates_lumin, i_lumin1, i_lumin2):
    ...
    deltaLum = np.subtract(sec_image, ref_image)  ## secondary minus reference. Because greater rfdi indicates deforestation and vice versa, higher deltaRFDI value will indicated greater change)
    ...
    deltaLum_outfilename = tmpName.replace("rfdi","DeltaRFDI")
    print("DeltaRFDI outfilename:", deltaLum_outfilename)
...
def parse_input_data(p):
    ...
    lum_dir = args['rfdi_file_loc']  
    output_directory = args['out_dir']
    lum_suffix = args['rfdi_suffix']
    coh_change_dir = args['coh_change_file_loc']
    coh_change_suffix = args['coh_change_suffix']
    lum_ref_img_fname = args['rfdi_ref_image']
...
if __name__ == "__main__":
    ...
    # Get list of coh Change input files (e.g. 20210212_20210224_20210308_cohChange.tif); DeltaRFDI will be calculated baesd on these images
    infile_coh_str = "*" + coh_change_suffix
    coh_images = glob.glob(os.path.join(coh_change_dir, infile_coh_str))

    # Get list of luminance images (e.g. S1B_IW_GRDH_1SDV_20210401_RFDI.tif)
